...
namespace Laura
{
    ...
    bool serializeState(const std::shared_ptr<const EditorState>& state) {
		...
		try {
			...
			if (!fout.is_open()) {
				LOG_EDITOR_CRITICAL("Could not open file for writing: {0}", EDITOR_STATE_FILE_PATH);
				return false;
			}

			fout << node;
			if (!fout) { // check write errors
				LOG_EDITOR_CRITICAL("Error occurred while writing file: {0}", EDITOR_STATE_FILE_PATH);
				return false;
			}
