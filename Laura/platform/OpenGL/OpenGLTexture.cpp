...
OpenGLTexture::OpenGLTexture(const std::string& filepath, uint16_t bindingPoint)
	...
	unsigned char* textureData = stbi_load(m_Filepath.c_str(), &m_Width, &m_Height, &m_Channels, m_BindingPoint);
	if (!textureData) {
...
OpenGLTexture::OpenGLTexture(int width, int height, uint16_t channels, uint16_t bindingPoint)
...
void OpenGLTexture::ChangeBindingPoint(uint16_t bindingPoint)
{
